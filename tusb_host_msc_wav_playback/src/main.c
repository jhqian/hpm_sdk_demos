...
#define task1_PRIORITY    (configMAX_PRIORITIES - 6U)
#define task2_PRIORITY    (configMAX_PRIORITIES - 3U)
#define task3_PRIORITY    (configMAX_PRIORITIES - 2U)
...
volatile bool disk_mounted = false;
...
static void task_tusb(void *pvParameters)
{
    print_greeting();
...
static void task_wavplay(void *ptr)
{
    uint32_t i = 0;
    char file_name[256];
    uint8_t input_end;
...
#else
    if (xTaskCreate(task_tusb, "task_tusb", 8192, NULL, task1_PRIORITY, NULL) != pdPASS) {
        printf("task_tusb creation failed!.\n");
        for (;;) {
            ;
        }
    }
    if (xTaskCreate(task_wavplay, "task_wavplay", 8192, NULL, task3_PRIORITY, NULL) != pdPASS) {
        printf("task_wavplay creation failed!.\n");
        for (;;) {
            ;
        }
    }
